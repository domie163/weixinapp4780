	...
	<div class="addEdit-block" :style='{"padding":"30px","background":""}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="科室名称" prop="keshimingcheng">
					<el-input v-model="ruleForm.keshimingcheng" placeholder="科室名称" clearable  :readonly="ro.keshimingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="科室名称" prop="keshimingcheng">
					<el-input v-model="ruleForm.keshimingcheng" placeholder="科室名称" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.keshitupian" label="科室图片" prop="keshitupian">
					<file-upload
						tip="点击上传科室图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.keshitupian?ruleForm.keshitupian:''"
						@change="keshitupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.keshitupian" label="科室图片" prop="keshitupian">
					<img v-if="ruleForm.keshitupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.keshitupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.keshitupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="症状描述" prop="zhengzhuangmiaoshu">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.zhengzhuangmiaoshu" label="症状描述" prop="zhengzhuangmiaoshu">
					<span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}'>{{ruleForm.zhengzhuangmiaoshu}}</span>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.zhenduanjieguo" label="诊断结果" prop="zhenduanjieguo">
					<span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}'>{{ruleForm.zhenduanjieguo}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="治疗方案" prop="zhiliaofangan">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.zhiliaofangan" label="治疗方案" prop="zhiliaofangan">
					<span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}'>{{ruleForm.zhiliaofangan}}</span>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='keshitupian'){
							this.ruleForm.keshitupian = obj[o];
	...
	if(this.ruleForm.keshitupian!=null) {
		this.ruleForm.keshitupian = this.ruleForm.keshitupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiuzhenjilu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiuzhenjiluCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    keshitupianUploadChange(fileUrls) {
	    this.ruleForm.keshitupian = fileUrls;
    },
  }
};
</script>
