...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyishengCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyishengCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussyisheng');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussyisheng');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussyisheng', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussyisheng', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussyisheng/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
