	...
	<view class="content">
		<view class="container" :style='{"minHeight":"100vh","width":"100%","padding":"0px 0 132rpx","position":"relative","background":"url() no-repeat right bottom,#f6f6f6","height":"auto"}'>
				...
				<view :style='{"margin":"0 0 20rpx 0","borderColor":"#ccc","borderRadius":"16rpx","borderWidth":"0 0 0px 0","background":"linear-gradient(180deg, rgba(246,246,246,1) 0%, rgba(103,197,254,1) 80%, rgba(6,159,250,1) 100%),#069ffa","display":"flex","width":"100%","borderStyle":"dashed","height":"auto"}' class="detail-list-item title">
					<view :style='{"padding":"0 20rpx 0 0","color":"#333","textAlign":"right","width":"188rpx","lineHeight":"80rpx","fontSize":"28rpx","minWidth":"188rpx"}' class="lable">器械名称：</view>
					<view :style='{"padding":"0px 20rpx 20rpx 0","margin":"16rpx 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}' class="text">{{detail.qixiemingcheng}}</view>
				</view>
				<view :style='{"margin":"0 0 20rpx 0","borderColor":"#ccc","borderRadius":"16rpx","borderWidth":"0 0 0px 0","background":"linear-gradient(180deg, rgba(246,246,246,1) 0%, rgba(103,197,254,1) 80%, rgba(6,159,250,1) 100%),#069ffa","display":"flex","width":"100%","borderStyle":"dashed","height":"auto"}' class="detail-list-item title">
					<view :style='{"padding":"0 20rpx 0 0","color":"#333","textAlign":"right","width":"188rpx","lineHeight":"80rpx","fontSize":"28rpx","minWidth":"188rpx"}' class="lable">器械类型：</view>
					<view :style='{"padding":"0px 20rpx 20rpx 0","margin":"16rpx 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}' class="text">{{detail.qixieleixing}}</view>
				</view>
				...
				<view class="detail-list-item" :style='{"margin":"0 0 20rpx 0","borderColor":"#ccc","borderRadius":"16rpx","borderWidth":"0 0 0px 0","background":"linear-gradient(180deg, rgba(246,246,246,1) 0%, rgba(103,197,254,1) 80%, rgba(6,159,250,1) 100%),#069ffa","display":"flex","width":"100%","borderStyle":"dashed","height":"auto"}'>
					<view class="lable" :style='{"padding":"0 20rpx 0 0","color":"#333","textAlign":"right","width":"188rpx","lineHeight":"80rpx","fontSize":"28rpx","minWidth":"188rpx"}'>生产商：</view>
					<view  class="text" :style='{"padding":"0px 20rpx 20rpx 0","margin":"16rpx 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}'>{{detail.shengchanshang}}</view>
				</view>
				<view class="detail-list-item" :style='{"margin":"0 0 20rpx 0","borderColor":"#ccc","borderRadius":"16rpx","borderWidth":"0 0 0px 0","background":"linear-gradient(180deg, rgba(246,246,246,1) 0%, rgba(103,197,254,1) 80%, rgba(6,159,250,1) 100%),#069ffa","display":"flex","width":"100%","borderStyle":"dashed","height":"auto"}'>
				...
				<view class="detail-list-item rich" :style='{"boxShadow":"0 8rpx 6rpx rgba(0,0,0,.1),inset 0px 0px 180rpx 0px #a0d2ef","padding":"16rpx 0px","margin":"0 0 24rpx 0","borderColor":"#91c8e9","borderRadius":"16rpx","borderWidth":"2rpx","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"188rpx","padding":"0 20rpx 0 0","lineHeight":"1.5","fontSize":"28rpx","color":"#333","textAlign":"right"}'>器械介绍</view>
					<view class="rich-text" :style='{"minHeight":"240rpx","border":"0px solid #f7de91","padding":"0 20rpx 0 0","boxShadow":"0 0px 0px rgba(182,158,18,.4),inset 0px 0px 0px 0px #fcf6d6","margin":"0","borderRadius":"8rpx","background":"none","width":"calc(100% - 8rpx)","lineHeight":"1.5"}'>
						<rich-text :nodes="detail.qixiejieshao"></rich-text>
					</view>
				</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			this.getThumbsup();
			let cleanType = uni.getStorageSync('discussqixiexinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussqixiexinxiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','qixiexinxi');
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.qixiemingcheng,
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','qixiexinxi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('qixiexinxi', this.id);
				this.detail = res.data;
				//修改富文本的图片样式
				this.detail.qixiejieshao = this.detail.qixiejieshao.replace(/img src/gi,"img style=\"width:100%;\" src");
				// 轮播图片
				this.swiperList = this.detail.qixietupian ? this.detail.qixietupian.split(",") : [];
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussqixiexinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
            ...
            async onCartTap() {
				...
				await this.$api.add('cart', {
					...
					goodname: this.detail.qixiemingcheng,
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussqixiexinxi/add-or-update?refid=${this.id}`)
			},
			// 获取赞踩
			async getThumbsup() {
				let params = {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('qixiexinxi', _this.detail);
								_this.$utils.msg('取消成功');
