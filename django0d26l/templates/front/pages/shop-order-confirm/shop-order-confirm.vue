					...
					success: async function(res) {
						if (res.confirm) {
							...
							for (let i = 0; i < _this.orderGoods.length; i++) {
                                ...
                                // 当前商品是否相同表
                                if(_this.orderGoods[i].tablename == `yaopinxinxi`){
                                     // 获取数量 
                                     let res = await _this.$api.info('yaopinxinxi', _this.orderGoods[i].goodid);
                                     // 单次限次
                                        ...
                                        break;
                                         ...
                                         if(_this.seat!=1){
                                            res.data.alllimittimes = res.data.alllimittimes - _this.orderGoods[i].buynumber;
                                             // 库存充足则减少库存数量
                                             await _this.$api.update(`yaopinxinxi`,  res.data);
                                         }else{
                                ...
                                // 当前商品是否相同表
                                if(_this.orderGoods[i].tablename == `qixiexinxi`){
                                     // 获取数量 
                                     let res = await _this.$api.info('qixiexinxi', _this.orderGoods[i].goodid);
                                     // 单次限次
                                     if(res.data.onelimittimes>0){
                                        // 超过单次限次购买数量
                                        if(_this.orderGoods[i].buynumber>res.data.onelimittimes){
                                     ...
                                     if(res.data.alllimittimes<_this.orderGoods[i].buynumber){
                                        ...
                                        break;
                                         ...
                                         if(_this.seat!=1){
                                            res.data.alllimittimes = res.data.alllimittimes - _this.orderGoods[i].buynumber;
                                             // 库存充足则减少库存数量
                                             await _this.$api.update(`qixiexinxi`,  res.data);
                                         }else{
                                            res.data.selected =  res.data.selected  + "," + _this.orderGoods[i].activeSeat
                                             if(isSuccess){
                                                 // 更新座位
